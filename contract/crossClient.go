// Copyright (c) 2025 The elastos developers
...
func (c *CrossClient) GetTransactionReceipt(txHash common.Hash) (*types.Receipt, error) {
	var receipt *types.Receipt
	err := c.client.CallContext(context.Background(), &receipt, "eth_getTransactionReceipt", txHash)
...
// GetBalance 获取账户余额
func (c *CrossClient) GetBalance(ctx context.Context, address common.Address) (*big.Int, error) {
	var result hexutil.Big
...
// GetCode 获取合约代码
func (c *CrossClient) GetCode(ctx context.Context, address common.Address) (string, error) {
	var code string
	err := c.client.CallContext(ctx, &code, "eth_getCode", address, "latest")
	if err != nil {
		return "", err
	}
	return code, nil
